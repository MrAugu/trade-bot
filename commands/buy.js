...
module.exports.run = async (client, message, args) => {
   ...
   const Option = await client.awaitReply(message, `What do you buy? **(item/world)** You have 30 seconds to anwer!`);
   if (["item"].includes(Option)) {
     const iName = await client.awaitReply(message, `What is the name of the item you buy?`);
     if(iName === false)
     return message.channel.send("Timed Out!");

     if(iName) {

       const isDesc = await client.awaitReply(message, `Please provide a short description of your advertisment! You have 30 seconds to anwer!`);

       if(isDesc === false)
       return message.channel.send("Timed Out!");

       if(isDesc) {
         const ilDesc = await client.awaitReply(message, `Please provide a short description of your advertisment! You have 30 seconds to anwer!`);
         ...
         if(ilDesc) {
           ...
           return message.channel.send("Your advertistment contains inappropriate words, request denied!");

           if(BdWd.includes(isDesc))
           return message.channel.send("Your advertistment contains inaapropriate words, request denied!");
           ...
           .setTitle(`${message.author.tag} (ID:${message.author.id}) is buying ${nam}!`)
           .setColor("#235adb")
   ...
   if (["world"].includes(Option)) {
     //WORLD
     const wName = await client.awaitReply(message, `What's the world name you buy? Ex:\`start\` You have 30 seconds to anwer!`);
     if(wName === false)
     return message.channel.send("Timed Out!");

     if(wName) {
       const wsDesc =  await client.awaitReply(message, `Please provide a short description of the advertisment, You have 30 seconds to anwer!`);
       if(wsDesc === false)
       return message.channel.send("Timed Out!");

       if(wsDesc) {
         const wlDesc =  await client.awaitReply(message, `Please provide a long description of the advertisment, please include as much detail. You have 30 seconds to anwer!`);
         if(wlDesc === false)
         return message.channel.send("Timed Out!");
         ...
         if(wlDesc) {
           ...
           .setTitle(`${message.author.tag} buys ${wName.toUpperCase()} world in growtopia!`)
           .setColor("#235adb")
